...
sh -c "echo \$$ >> /cgroup/${hierarchy01}/${cg02}/tasks && dd if=/dev/zero of=${testfile_path02}" &
...
trap "kill -9 $pid01 $pid02 >/dev/null 2>&1;rm -rf ${testfile_path01} ${testfile_path02}" EXIT
...
[ -f /tmp/${tmp_file} ] && trap "kill -9 $pid01 $pid02 >/dev/null 2>&1;rm -rf ${testfile_path01} ${testfile_path02} /tmp/${tmp_file}" EXIT
